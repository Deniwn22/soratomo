  ...
  return (
    <div onClick={e=>e.stopPropagation()} style={{
      ...
      {f.confidence&&(()=>{
        ...
        const desc={HIGH:'AR pointing accurate · < 3.5° angular error',MED:'Normal accuracy · 3.5–8.5° angular error',LOW:'Low confidence · stale data or compass drift'}[f.confidence];
        return (
...
export default function App() {
  ...
  const mapped=visibleFlights.map(f=>{
    ...
    //   < 3.5° → HIGH  (well-calibrated compass, fresh data, or aircraft close)
    //   3.5–8.5° → MED   (typical compass + some DR age — normal conditions)
    //   > 8.5° → LOW   (stale DR on a fast aircraft, or significant compass error)
    const confidence = angUncertDeg < 3.5 ? 'HIGH' : angUncertDeg < 8.5 ? 'MED' : 'LOW';
